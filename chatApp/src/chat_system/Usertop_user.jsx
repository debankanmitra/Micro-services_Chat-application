import React from 'react'
import styled from 'styled-components'

const Div = styled.div`
  font-size: 20px;
  display: flex;
  gap: 10px;
`
const Div1 = styled.div`
  
`
const Div2 = styled.div`
  
`
const H1 = styled.div`
  font-size: 20px;
  font-weight: 600;
`
const H2 = styled.div`
  font-size: 10px;
  display: flex;
`

function Usertop_user() {
  return (
    <Div>

      <Div1>
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="#0079FF"
          d="M16 8a5 5 0 1 0 5 5a5 5 0 0 0-5-5Z" /><path fill="#0079FF" d="M16 2a14 14 0 1 0 14 14A14.016 14.016 0 0 0 16 2Zm7.992 22.926A5.002 
        5.002 0 0 0 19 20h-6a5.002 5.002 0 0 0-4.992 4.926a12 12 0 1 1 15.985 0Z"/></svg>
      </Div1>

      <Div2>
        <H1>Louis</H1>
        <H2><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24">
          <path fill="currentColor" d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10s10-4.47 10-10S17.53 2 12 2z" /></svg> Active</H2>
      </Div2>


    </Div>
  )
}

export default Usertop_user